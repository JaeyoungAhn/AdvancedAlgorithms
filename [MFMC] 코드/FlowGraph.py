...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        lenient_win = self.wins[self.team2id[teamName]] + self.remaining[self.team2id[teamName]]
        result_list = []

        for i in range(len(self.teams)):
            if(teamName == self.teams[i]):
                continue
            if lenient_win < self.wins[i]:
                result_list.append(self.teams[i])
        ...
        """ combination """
        combi_numerator = 1
        for i in range(1,len(self.teams)): # 자신의 팀 제외
            combi_numerator = combi_numerator * i
        combi_denominator = 1
        for i in range(1, len(self.teams)-2): # nC2
            combi_denominator = combi_denominator * i
        combination = int(combi_numerator/(combi_denominator*2)) # n!/(n-r)!*r!
        # 5 개 4 -- 1 2 3 4  -- 1 2, 1 3, 1 4, 2 3, 2 4, 3 4, 4C2 4 3/ 2 1
        fn = FlowNetwork(combination+len(self.teams)-1+2) # 조합개수+전체팀-대상팀+2(s,t)
        match = []

        for i in range(0, len(self.teams)):
            if i == self.team2id[teamName]: # 자신의 팀인 경우
                continue
            for j in range(0, len(self.teams)):
                if j == self.team2id[teamName]: # 자신의 팀과 붙는 경우
                    continue
                if i == j: # 둘 다 동일한 팀인 경우
                    continue
                if [i, j, self.against[i][j]] in match or [j, i, self.against[i][j]] in match:
                    continue
                match.append([i, j, self.against[i][j]])

        idx = 0
        for i in range(1, combination+1): # s에서 각 매치 정점까지 이어주기
            fn.addEdge(FlowEdge(0,i,match[idx][2]))
            idx = idx + 1

        match_idx = 0
        for each_match in match: # 각 매치 정점에서 팀 정점까지 이어주고 중간에 제외 팀 index 넘어서면 -1
            if self.team2id[teamName] <= each_match[0]:
                fn.addEdge(FlowEdge(1+match_idx, combination+1+each_match[0]-1,float("inf")))
            else:
                fn.addEdge(FlowEdge(1+match_idx, combination+1+each_match[0],float("inf")))

            if self.team2id[teamName] <= each_match[1]:
                fn.addEdge(FlowEdge(1+match_idx, combination+1+each_match[1]-1,float("inf")))
            else:
                fn.addEdge(FlowEdge(1+match_idx, combination+1+each_match[1],float("inf")))
            match_idx = match_idx + 1

        each_team = 0
        for k in range(combination+1, combination+len(self.teams)-1+2-1): # 팀 정점에서 t까지 연결
            if each_team == self.team2id[teamName]: # 불러낸 팀은 제외
                each_team = each_team + 1
            fn.addEdge(FlowEdge(k, combination+len(self.teams)-1+2-1, lenient_win - self.wins[each_team]))
            each_team = each_team + 1

        ff = FordFulkerson(fn, 0, combination+len(self.teams)-1+2-1)
        flag = 0
        for one_team in self.teams:
            if one_team == teamName:
                flag = 1
                continue
            if flag == 0 and ff.inCut(combination+1+self.team2id[one_team]):
                result_list.append(one_team)
            elif flag == 1 and ff.inCut(combination+1+self.team2id[one_team]-1):
                result_list.append(one_team)
