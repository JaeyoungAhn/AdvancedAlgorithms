...
def sweepLine(segments):
    ...
    while minPQ.qsize() > 0:
        ...
        if segment.isHorizontal():
            if RBTree.contains(segment.y1):
                RBTree.delete(segment.y1)
            else:
                RBTree.put(segment.y1, segment)
        elif segment.isVertical():
            q = RBTree.rangeSearch(segment.y1, segment.y2)
            for ypoint in q:
                Result.append((RBTree.get(ypoint), segment))
