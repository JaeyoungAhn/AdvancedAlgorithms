...
class SeamCarver:
    ...
    def findVerticalSeam(self):
        distTo = [[self.MAX_ENERGY for i in range(self.width())]]
        edgeTo = [[None for i in range(self.width())]]
        ...
        for y in range(1, self.height()):
            ...
            for x in range(self.width()):
                ...
                prevX = x
                if x-1 >= 0 and minEnergy > distTo[y-1][x-1]:
                    minEnergy = distTo[y-1][x-1]
                    prevX = x-1
                if x+1 < self.width() and minEnergy > distTo[y-1][x+1]:
                    minEnergy = distTo[y-1][x+1]
                    prevX = x+1
                distTo[y][x] = minEnergy + self.energy(x,y)
                edgeTo[y][x] = prevX
        ...
        for i in range(self.width()):
            if minEnergy > distTo[self.height()-1][i]: # 같은 경우?
                minEnergy = distTo[self.height()-1][i]
                x_min = i

        for y in range(self.height()-1, -1, -1):  # (h-1) ~ 0
            seam[y] = x_min
            if y != 0:
